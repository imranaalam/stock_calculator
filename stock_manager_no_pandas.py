...
def view_stocks():
    ...
    table_data = []
    headers = [
        'Stock Symbol', 'Total Shares', 'Buy Price ($)', 'Risk Ratio (%)',
        'Reward Ratio (%)', 'Sell Strategy', 'Total Investment ($)',
        'Risk Amount ($)', 'Reward Amount ($)', 'Stop-Loss Price ($)',
        'Take-Profit Price ($)', 'Adjusted Sell Price ($)'
    ]
    ...
    for entry in st.session_state.entries:
        ...
        row = [
            ...
            f"{entry.risk_ratio:.2f}%",
            f"{entry.reward_ratio:.2f}%",
            entry.sell_strategy,
            f"${metrics['Total Investment ($)']:.2f}",
            f"${metrics['Risk Amount ($)']:.2f}",
            f"${metrics['Reward Amount ($)']:.2f}",
            f"${metrics['Stop-Loss Price ($)']:.2f}",
            f"${metrics['Take-Profit Price ($)']:.2f}",
            f"${metrics['Adjusted Sell Price ($)']:.2f}"
        ]
